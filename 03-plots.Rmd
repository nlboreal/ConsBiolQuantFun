...
In this section, we will make a few different types of graphs, using the package ```ggplot2```. You'll also spend some time thinking abou the best way to visualize different types of data.
...
Then we simply add another ```geom``` function to the code snippet above.
...
1. Pick two variables in one of the datasets we're using here that woudl be best shown as a scatterplot. Feel free to join or group data to practcie that skill. Try adjusting the symbology and axis lables for maximum effect. Hand in a copy of your code and the finished figure. Justify what a scatterplot is an appropriate way to display these data. 

1. **For graduate students** In addition to the above step, pick a graph type in ```ggplot2``` that you have never created before (but that perhaps is a way you may want to visualize your own data for your graduate work), and pick the appropriate data from the sets avaialbe here to create a figure. Hand in a copy of your code and the finished figure. Justify why the data chosen fit this kind of figure well. 
